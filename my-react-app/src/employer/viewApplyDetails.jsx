  ...
  const updateApplicationStatus = async (status) => {
    try {
      const response = await axios.patch(
        `http://localhost:8000/api/UpdateApplicationStatus/${apply.id}`,
        { status },
        {
          headers: {
    ...
    <div className="addrole fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-4 rounded shadow-lg relative w-full max-w-2xl max-h-full">
        ...
        <div className="appScroll h-[500px] overflow-y-auto mr-[-16px]">
          <div>
            <h1 className="text-xl font-semibold uppercase text-neutral-600">
              User Information
            </h1>
              ...
              <div className="flex items-center gap-5">
                {apply.user.profilePic ? (
                  <img
                    ...
                    alt=""
                    className="rounded-full h-[100px] w-[100px] border border-neutral-700"
                  />
                ) : (
            ...
            {myConstants.map((item, index) => (
              <div key={index} className="mb-[20px]">
                <h1 className="font-bold text-xl mb-[10px] underline underline-offset-4">
                  {item.title}
                </h1>
          ...
          <div>
            ...
            {apply.user.skills ? (
              ...
              <p className="text-lg">
                Job seeker has not added any skills yet.
              </p>
            )}
          </div>
        </div>
        <form className="flex justify-center mb-[-16px] mx-[-16px] bg-stone-200 rounded p-2 space-x-4">
          <button
            ...
            className="border border-neutral-700 px-4 py-2 bg-red-500 text-white"
          >
            Reject
          </button>
          <button
            ...
            className="border border-neutral-700 px-4 py-2 bg-green-500 text-white"
          >
            Accept
          </button>
        </form>
